<template>
  ...
  <movie-detail v-if="showModal" :id="Vmodal.id" @close="showModal = false">
    ...
    <b-container>
        <h3 class="mb-4 pageHeader">Vizyondaki Filmler</h3>
         <b-row  >
                <b-col lg="6" class="mb-3" v-for="entry in nowplaying.results" :key="entry.id">
                     <div >
...
export default {
  ...
  methods: {
    ...
    getTotalRecord () {
      axios.get('https://api.themoviedb.org/3/movie/now_playing?api_key=f9261403d3de49a0151e3debf139d4b6&language=tr&page=1')
      .then(response => {
        this.totalRecords = parseInt(response.data.total_results)
      })
    },
    getPageData (page) {
      axios.get('https://api.themoviedb.org/3/movie/now_playing?api_key=f9261403d3de49a0151e3debf139d4b6&language=tr&page='+page)
      .then(response => {
        this.nowplaying = response.data
      })
    },
    myCallback () {
      this.$router.push({ name: 'NowPlaying', params: {page: this.currentPage}})
